 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 view=glm::mat4(1);
	      ...
	      view = glm::translate(view, glm::vec3(1.0f,-6.0f,-15.0f)); //unicamente se mueve la vista (0.0f,-8.0f,-15.0f)
          //view = glm::rotate(view, 0.5f, glm::vec3(1.0f, -20.0f, 0.0f));
		...
		glUniform1d(colorLoc, 0.5f); //cambiamos el color del primer cubito

		glBindVertexArray(VAO);
		glDrawArrays(GL_TRIANGLES, 0, 36);


		model = glm::mat4(1); // inicializamos la matriz
		model = glm::translate(model,glm::vec3(1.0f, 2.0f, 0.0f));
		model = glm::rotate(model, 0.5f, glm::vec3(0.0f, 1.0f, 0.0f)); //
		model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f));

		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); // mandamos la informacion al shader
			//cambiamos el color del segundo cubito
		glDrawArrays(GL_TRIANGLES, 0, 36); // dibujamos el segundo cubo
		...
		model = glm::mat4(1); // inicializamos la matriz
		model = glm::translate(model, glm::vec3(3.0f, 4.5f, 1.0f));
		model = glm::rotate(model, 0.5f, glm::vec3(0.0f, -2.0f, 0.0f)); //
		model = glm::scale(model, glm::vec3(2.0f, 1.0f, 1.5f));

		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); // mandamos la informacion al shader
		//cambiamos el color del segundo cubito
		glDrawArrays(GL_TRIANGLES, 0, 36); // dibujamos el cuarto cubo
		

		model = glm::mat4(1); // inicializamos la matriz
		model = glm::translate(model, glm::vec3(3.0f, 5.5f, 1.75f));
		model = glm::rotate(model, 0.5f, glm::vec3(3.0f, 0.0f, 8.0f)); //
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 0.5f));

		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); // mandamos la informacion al shader
		//cambiamos el color del segundo cubito
		glDrawArrays(GL_TRIANGLES, 0, 36); // dibujamos el quinto cubo
